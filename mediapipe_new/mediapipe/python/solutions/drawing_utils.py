...
def draw_detection(
  ...
  def mosaic_area(img, start,end, ratio=0.05):
    img_c=img.copy()
    width=end[0]-start[0]
    height=end[1]-start[1]
    if start[1]-height/2>0:
      img_c[start[1]-math.floor(height/2):start[1], start[0]:start[0] + abs(width)] = mosaic(img[start[1]-math.floor(height/2):start[1], start[0]:start[0] + abs(width)],ratio)
    elif start[1]-height/2<0:
      img_c[0:start[1], start[0]:start[0] + abs(width)] = mosaic(img[0:start[1], start[0]:start[0] + abs(width)],ratio)
    img_c[start[1]:start[1] + abs(height), start[0]:start[0] + abs(width)] = mosaic(img[start[1]:start[1] + abs(height), start[0]:start[0] + abs(width)],ratio)
    return img_c
  dst_face = mosaic_area(image,rect_start_point,rect_end_point)
  #import pdb; pdb.set_trace() 
  return dst_face

'''
  w = -rect_start_point[0]+rect_end_point[0]
  h = -rect_start_point[1]+rect_end_point[1]
  x = rect_start_point[0]
  y = rect_start_point[1]-int(math.floor(h/3))
  if y<0:
    y=0
